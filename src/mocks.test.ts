...
import { Server as MockSever, WebSocket as ServerWebsocket } from 'mock-socket';
...
describe('data-mocks', () => {
  ...
  describe('Websockets', () => {
    ...
    // describe('Mock from scenario', () => {
    //   it('works', async () => {
    //     const onMessage = jest.fn()
    //     const onConnect = (socket: ServerWebsocket) => socket.on('message', onMessage)
    //     const onConnectSpy = jest.spyOn({ onConnect }, 'onConnect')
    //     const onMessageSpy = () => jest.spyOn({ onMessage }, 'onMessage')
    ...
    //     const socket = new WebSocket('/foo')
    //     await awaitSocket(socket, WebSocket.OPEN)
    //     expect(onConnectSpy).toBeCalled()
...
const createServer = () => {
  const testURI = 'ws://localhost/foo';
  const testServerMock = (server: MockSever) => (WebSocketServerMock) => {
